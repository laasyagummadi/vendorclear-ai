...
class TestAdminOnlyAccess:
    async def test_vendor_cannot_read_config(self, client: AsyncClient, auth_headers):
        # auth_headers is a default VENDOR-role user
        resp = await client.get("/api/v1/config/versions", headers=auth_headers)
        assert resp.status_code == 403

    async def test_vendor_cannot_update_config(self, client: AsyncClient, auth_headers):
