...
class ApprovalService:
    ...
    # ── Policies ──────────────────────────────────────────────
    async def create_policy(self, data: dict, created_by_id: Optional[str]) -> CompliancePolicy:
    ...
    async def list_policies(self, version: Optional[int] = None, active_only: bool = True):
        stmt = select(CompliancePolicy)
        if version is not None:
            stmt = stmt.where(CompliancePolicy.version == version)
        if active_only:
            stmt = stmt.where(CompliancePolicy.is_active == True)  # noqa: E712
    ...
    async def get_policy(self, policy_id: str) -> Optional[CompliancePolicy]:
        return await self.db.get(CompliancePolicy, policy_id)

    async def update_policy(self, policy: CompliancePolicy, updates: dict) -> CompliancePolicy:
    ...
    async def check_requirements(self, vendor_id: str, policy: CompliancePolicy) -> tuple[bool, dict]:
        """
    ...
    async def list_submissions(
        ...
        stmt = select(PolicySubmission)
        if status:
            stmt = stmt.where(PolicySubmission.status == status)
        if vendor_id:
            stmt = stmt.where(PolicySubmission.vendor_id == vendor_id)
        stmt = stmt.order_by(PolicySubmission.created_at.desc())
        return list((await self.db.execute(stmt)).scalars().all())

    async def get_submission(self, submission_id: str) -> Optional[PolicySubmission]:
        return await self.db.get(PolicySubmission, submission_id)

    async def transition(
        self, submission: PolicySubmission, new_status: SubmissionStatus,
