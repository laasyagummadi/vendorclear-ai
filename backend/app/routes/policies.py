...
from app.schemas.approvals import (
    ...
    ApprovalSubmissionCreate, ApprovalSubmissionDecision, ApprovalSubmissionOut,
)
...
# ── Policies ──────────────────────────────────────────────────
@router.post("", response_model=ApprovalPolicyOut, status_code=201,
...
@router.get("", response_model=List[ApprovalPolicyOut], summary="List compliance policies")
...
@router.patch("/{policy_id}", response_model=ApprovalPolicyOut,
...
# ── Submissions ───────────────────────────────────────────────
@router.post("/submissions", response_model=ApprovalSubmissionOut, status_code=201,
             summary="Submit a vendor against a policy for approval")
async def create_submission(
    data: ApprovalSubmissionCreate,
    db: AsyncSession = Depends(get_db),
...
@router.get("/submissions", response_model=List[ApprovalSubmissionOut],
...
@router.get("/submissions/{submission_id}", response_model=ApprovalSubmissionOut,
...
async def get_submission(
    ...
    if not has_permission(user, Permission.VENDOR_VIEW_ALL):
        if sub.vendor_id != user.vendor_id:
            raise HTTPException(status_code=403, detail="Access denied.")
    return sub


@router.post("/submissions/{submission_id}/approve", response_model=ApprovalSubmissionOut,
             summary="Approve a submission (admin only)")
async def approve_submission(
    submission_id: str,
    data: ApprovalSubmissionDecision,
    db: AsyncSession = Depends(get_db),
    ...
    try:
        return await svc.transition(
            sub, SubmissionStatus.APPROVED, reviewer_id=user.id, notes=data.notes
        )
    except ValueError as e:
        raise HTTPException(status_code=409, detail=str(e))


@router.post("/submissions/{submission_id}/reject", response_model=ApprovalSubmissionOut,
...
async def reject_submission(
    ...
    try:
        return await svc.transition(
            sub, SubmissionStatus.REJECTED, reviewer_id=user.id, notes=data.notes
        )
    except ValueError as e:
        raise HTTPException(status_code=409, detail=str(e))
