# ─────────────────────────────────────────────────────────────
#  app/routes/policies.py  —  Compliance policy admin endpoints
# ─────────────────────────────────────────────────────────────
"""
Admin-configurable per-category compliance scoring policies. This is
what makes "Dynamic compliance rules" dynamic: Construction, Software,
and Electrical vendors can each be scored with different weightings,
required document types, finding penalties, and risk thresholds
instead of one hardcoded formula applied to every vendor.
"""
...
# Feature 7 (RBAC): only Admins configure compliance policies — every other
# role (Analyst/Vendor/Auditor) can still GET/list them (read-only) below.
